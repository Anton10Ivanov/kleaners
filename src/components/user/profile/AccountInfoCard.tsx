...
export function AccountInfoCard({
  ...
  return <Card>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              {isEditing ? <Input id="fullName" name="fullName" value={formData.fullName} onChange={handleChange} required /> : <div className="py-2">{profile.fullName}</div>}
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              {isEditing ? <Input id="email" name="email" type="email" value={formData.email} onChange={handleChange} required /> : <div className="py-2">{profile.email}</div>}
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="phone">Phone Number</Label>
              {isEditing ? <Input id="phone" name="phone" value={formData.phone} onChange={handleChange} /> : <div className="py-2">{profile.phone || "Not provided"}</div>}
            </div>
          </div>
        </form>
      </CardContent>
      ...
      <CardFooter className="flex justify-end">
        {isEditing ? <>
            <Button variant="outline" onClick={() => setIsEditing(false)} className="mr-2" type="button">
              Cancel
            </Button>
