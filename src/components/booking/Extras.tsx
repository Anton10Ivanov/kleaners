...
const Extras = ({
  ...
  return <div className="space-y-6">
      <h3 className="text-xl text-center mb-6 font-normal">Additional Services</h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {AVAILABLE_EXTRAS.map(extra => {
        const Icon = extra.icon;
        const isSelected = selectedExtras.includes(extra.id);
        return <Button key={extra.id} variant="outline" className={`
              ...
              <div className="text-center space-y-2 w-full">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs font-semibold">{extra.title}</span>
                </div>
                {extra.time && <div className="text-secondary-text text-sm">{extra.time}</div>}
              </div>
            </Button>;
      })}
      </div>
        ...
        <DialogContent>
          ...
          <div className="py-4">
            <div className="flex items-center justify-center gap-4">
              <Button variant="outline" onClick={() => setWindowCount(Math.max(1, windowCount - 1))} disabled={windowCount <= 1}>
                -
              </Button>
              <span className="w-16 text-center font-medium">{windowCount}</span>
              <Button variant="outline" onClick={() => setWindowCount(windowCount + 1)}>
                +
              </Button>
            </div>
      ...
      <Dialog open={openDialog === 'ironing'} onOpenChange={() => setOpenDialog(null)}>
        <DialogContent>
          ...
          <div className="py-4">
            <div className="flex items-center justify-center gap-4">
              <Button variant="outline" onClick={() => setIroningTime(Math.max(30, ironingTime - 30))} disabled={ironingTime <= 30}>
                -
              </Button>
              <span className="w-16 text-center font-medium">{ironingTime} min</span>
              <Button variant="outline" onClick={() => setIroningTime(Math.min(240, ironingTime + 30))} disabled={ironingTime >= 240}>
                +
              </Button>
            </div>
            <div className="mt-6 flex justify-end">
