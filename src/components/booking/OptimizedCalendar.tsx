...
const OptimizedCalendar = ({
    ...
    const fetchAvailableProviders = async () => {
      ...
      try {
        ...
        const {
          ...
          `).eq('status', 'approved').contains('provider_service_areas.postal_code', [postalCode]);
        if (error) {
  ...
  return <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-gray-700">
          ...
          <span className="font-medium text-gray-900 dark:text-white">
            {format(weekStart, 'MMM d')} - {format(endOfWeek(weekStart, {
            weekStartsOn: 1
          }), 'MMM d, yyyy')}
          </span>
        ...
        <div className="grid grid-cols-7 divide-x divide-gray-200 dark:divide-gray-700">
          ...
          return <button key={index} onClick={() => !isDisabled && handleDateSelect(weekDate)} disabled={isDisabled} className={`
                ...
                {isToday && <div className={`w-1 h-1 rounded-full mx-auto mt-1 ${isSelected ? 'bg-white' : 'bg-primary'}`} />}
              </button>;
        })}
        </div>
      </div>
      ...
      y: 0
          ...
          <TimeSlider value={currentStartTime} onChange={handleTimeSlotSelect} />
        </motion.div>}

      {/* Provider Availability */}
      {selectedTimeSlot && <motion.div initial={{
      opacity: 0,
      y: 10
    }} animate={{
      opacity: 1,
      y: 0
            ...
            ${availableProviders.length > 0 ? 'bg-green-50 dark:bg-green-900/20 border-green-400' : 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-400'}
          `}>
          <div className="flex items-center gap-2">
            <Users className="h-4 w-4" />
            <span className="text-sm font-medium">
              {availableProviders.length > 0 ? `${availableProviders.length} provider${availableProviders.length !== 1 ? 's' : ''} available` : 'No providers currently available'}
            </span>
          </div>
          {availableProviders.length === 0 && <p className="text-xs text-yellow-700 dark:text-yellow-300 mt-1">
              Your booking will be matched with the next available provider.
            </p>}
        </motion.div>}

      {!selectedTimeSlot && date && <p className="text-sm text-gray-600 dark:text-gray-400 text-center">
