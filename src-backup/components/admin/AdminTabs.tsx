...
const tabSections = [
  ...
  { id: 'schedule', label: 'Schedule Management', component: ScheduleManagement },
];
...
const AdminTabs = ({ defaultTab = 'questions', children }: AdminTabsProps) => {
  ...
  return (
    <Tabs defaultValue={activeTab} onValueChange={handleTabChange} className="form-spacing-relaxed">
      ...
      {tabSections.map((section) => (
        <TabsContent key={section.id} value={section.id} className="form-spacing-relaxed">
          <section.component />
        </TabsContent>
      ))}
