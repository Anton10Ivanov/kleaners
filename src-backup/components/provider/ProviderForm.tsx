...
import { Save, X, RefreshCw, Upload, Calendar, Clock, MapPin, User, Phone, Mail } from 'lucide-react';
import { cn } from '@/lib/utils';

export interface ProviderFormFieldConfig {
  name: string;
  label: string;
  type: 'text' | 'email' | 'password' | 'number' | 'textarea' | 'select' | 'checkbox' | 'switch' | 'date' | 'url' | 'file' | 'time' | 'multiselect';
  placeholder?: string;
  required?: boolean;
  disabled?: boolean;
...
export interface ProviderFormProps<T extends FieldValues = FieldValues> {
  title?: string;
  description?: string;
  fields: ProviderFormFieldConfig[];
  ...
  type?: 'profile' | 'availability' | 'application' | 'settings';
}

export function ProviderForm<T extends FieldValues = FieldValues>({
  title,
  description,
  fields,
  schema,
  ...
  const getTypeIcon = () => {
    switch (type) {
      ...
      case 'application': return <Upload className="h-5 w-5" />;
      case 'settings': return <Phone className="h-5 w-5" />;
      default: return <User className="h-5 w-5" />;
    }
  };

  const renderField = (field: ProviderFormFieldConfig) => {
    const fieldWidth = {
      full: 'col-span-1',
